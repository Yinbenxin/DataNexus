...
class BenchmarkEmbeddingAPI(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        # 从URL中解析主机和端口
    ...
    def create_embedding_task(self, text: str) -> Dict[str, Any]:
        """创建embedding任务并等待结果"""
    ...
    def test_text_length_performance(self):
        ...
        test_texts = [
            "这是一个较长测试文本",  # 4个字
            "这是一个较长测试文本" * 10,  # ~80个字
            "这是一个很长测试文本" * 100,  # ~400个字
        ]

        results = []
        for text in test_texts:
            start_time = time.time()
            result = self.create_embedding_task(text)
            end_time = time.time()
    ...
    def test_concurrent_requests(self):
        """测试并发请求的响应时间"""
        concurrent_counts = [10]
        test_text = "这是一个并发测试文本"
        results = []

        def make_request():
            start_time = time.time()
            result = self.create_embedding_task(test_text)
            end_time = time.time()
    ...
    def test_memory_usage(self):
        ...
        # 发送一系列请求并监控内存使用
        test_text = "这是一个内存使用测试文本" * 10
        request_count = 10
        memory_samples = [initial_memory]

        for i in range(request_count):
            self.create_embedding_task(test_text)
